...
dependencies needed for running microbiolink (`[tool.poetry.dependencies]` with the
...
developing microbiolink using test-driven development. Please make sure that you
...
If you are brand new to microbiolink or open-source development, we recommend
...
microbiolink is hosted on GitHub, and to contribute, you will need to sign up for a
...
### Create a fork of microbiolink

You will need your own fork of microbiolinkin order to eventually open a Pull
Request. Go to the microbiolink project page and hit the Fork button. Please
...
```bash
git clone https://github.com/your-user-name/microbiolink.git
cd microbiolink
git remote add upstream https://github.com/microbiolink/microbiolink.git
git fetch upstream
```

This creates the directory `microbiolink` and connects your repository to the
upstream (main project) *microbiolink* repository. They have the same name, but
your local repository and fork are separate from the upstream repository.

### Creating a feature branch

Your local `main` branch should always reflect the current state of microbiolink
repository. First ensure it's up-to-date with the main microbiolink repository.
...
to create a branch called `my-new-feature-for-microbiolink`

```bash
git checkout -b my-new-feature-for-microbiolink
```

This changes your working branch from `main` to the
`my-new-feature-for-microbiolink` branch. Keep any changes in this branch specific
to one bug or feature so it is clear what the branch brings to *microbiolink*. You
...
When making changes, follow these microbiolink-specific guidelines:
...
Now your code is on GitHub, but it is not yet a part of the microbiolink project.
...
1. Write a descriptive title that includes prefixes. microbiolink uses a convention for title prefixes, most commonly, `feat:` for features, `fix:` for bug fixes, and `refactor:` for refactoring.
...
branch with changes in the microbiolink `main` branch, run:

```shell

    git checkout my-new-feature-for-microbiolink
    git fetch upstream
...
```shell
    git push origin my-new-feature-for-microbiolink
```
