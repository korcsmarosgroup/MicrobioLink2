...
Detailed plan for the fifth module described in @ai_docs/plans/microbiolink_refactoring.md, following the
decisions in @ai_docs/plans/refactoring_questions.md and the precedent set by
@ai_docs/plans/module_1_zscore_filter.md through @ai_docs/plans/module_4_download_domains.md. Scope is exactly
...
`case-study`** — it is spec-only there (see `ai_docs/plans/microbiolink_refactoring.md`'s Module 5
...
- Beta's input shape is **domain-keyed**: `dict[pfam_id -> [protein_id, ...]]`, produced by
  `microbiolink_api/dmi.py`'s `read_protein_domain_table`. **@ai_docs/plans/module_4_download_domains.md
...
- [x] Confirm `microbiolink/workflow/domain_download.py` has been updated per
      @ai_docs/plans/module_4_download_domains.md to return the Pfam-keyed shape (prerequisite for this
      module).
