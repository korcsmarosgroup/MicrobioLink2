...
def parse_arguments():
    ...
    parser.add_argument("--sep", required=True, help="Separator for the background gene list file")
    parser.add_argument("--target_gene_list", required=True, help="Path to the target gene list file.")
    parser.add_argument("--analysis_level", required=False, help="Type of target gene set analysis. HMI option anlyses proteins targeted by bacterial interactions. TieDIE option focuses on the downstream signaling network")
...
def read_target_gene_list_HMI(file_path):
    ...
    with open(file_path, "r") as target_gene_list:
        for line in target_gene_list:
            ...
            target_geneset.extend([line[0], line[5]])
...
def main():
    ...
    background_expressed_gene_symbols = read_background_gene_list(args.background_gene_list, args.sep)

    if args.analysis_level == 'HMI':
        target_geneset = read_target_gene_list_HMI(args.target_gene_list)

    if args.analysis_level == 'TieDIE':
