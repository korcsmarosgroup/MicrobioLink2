...
class DMIResourceBundle:
    ...
    @property
    def motif_regex(self) -> dict[str, str]:
        """Return the motif-pattern mapping used by the bundle."""

        return self.elm_regex
...
def read_protein_domain_table(filename: PathLike) -> dict[str, list[str]]:
    """Read proteins grouped by Pfam domain.

    Args:
        filename: Path to a two-column protein/domain table.

    Returns:
        A mapping from Pfam domain to proteins containing that domain.
    """
    ...
    with open(filename, encoding = 'utf-8') as protein_domain_file:
        ...
        for line in protein_domain_file:
            ...
            for pfam_domain in fields[1].split(';'):
                protein_domains.setdefault(pfam_domain, []).append(fields[0])
...
def read_bacterial_domain_table(filename: PathLike) -> dict[str, list[str]]:
    ...
    return read_protein_domain_table(filename)
